...
fit_tree_test <- rpart(shares~., data=mydf[testloc,],parms="gini",control=rpart.control(
  minsplit =5))
...
varImpPlot(onlinenews.rf.test, sort=TRUE, n.var=min(10, nrow(onlinenews.rf.test$importance)),
           type=NULL, class=NULL, scale=TRUE, 
           main="Variable Importance Plot")
### estimate using the model
predict_test_rf<-predict(onlinenews.rf.test, data=mydf[testloc,])
